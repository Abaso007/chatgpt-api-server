...
class OpenAIAuth:
    ...
    def __part_seven(self, state: str) -> None:
        ...
        if response.status_code == 302:
            ...
            self.debugger.log("Status code: ", end="")
            self.debugger.log(response.status_code)
            wrong_combo_substring = 'Wrong email or password'
            if wrong_combo_substring in response.text:
                self.access_token = 'Wrong email or password provided'
            else: 
    ...
    async def get_access_token(self):
        ...
        Gets access token
        """
        if self.access_token == 'Wrong email or password provided':
